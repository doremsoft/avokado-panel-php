...
Vvveb.baseUrl =server_editor_builder_url;
				...
				for (i in components)
				{
					...
					if (component)
					{
						...
						if (component.image) {
							...
							item.css({
								backgroundImage: "url(" +server_editor_builder_url+""+ component.image + ")",
...
/* drag and drop */
	...
	newPageAjax: function(fileName, startTemplateUrl, callback)
	{
		var data = {};
	...
	saveAjax: function(fileName, startTemplateUrl, callback)
	{
		...
		$.ajax({
			type: "POST",
			url: server_save_url,//set your server side save script url
			data: data,
			cache: false,
			success: function (data) {
...
//Pages, file/components tree 
	newPage : function () {
		...
		newPageModal.modal("show").find("form").off("submit").submit(function( event ) {
			...
			return Vvveb.Builder.newPageAjax(fileName, startTemplateUrl, function (data) {
