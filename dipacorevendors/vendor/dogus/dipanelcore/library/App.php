...
class App
{
    ...
    public function __construct($terminal = false)
    {
        ...
        $this->setConfig();
        $system_type = self::getConfig("system_type");


        if ($system_type == "panel") {

            if (trim(file_get_contents(BASE_PATH . "bakimmod.php")) == 1) {

                include(PUBLIC_DIR . DS . "bakimmodu.html");
    ...
    private function setConfig()
    {
        ...
        if ($configs = include(BASE_PATH . "config.php")) {
    ...
    public function run()
    {
        ...
        require BASE_PATH . 'routerList.php';


        $this->route->any('/*', function () {
            \Dipa\View::render("error/404");
        });
