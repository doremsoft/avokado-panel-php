...
$private_key_avokado = base64_encode(uniqid());
...
$account_no = "0";
$user_remove_file = true;
$user_rename_file = true;
$user_cretate_file = true;
$user_upload_file = true;
$user_move_file = true;
$user_rename_folder = true;
$user_edit_file = true;
$user_upload_file = true;
$klasor_limit = 256;
$toplam_tanimli_depolama_alani = 0;


if(isset($_GET["klasor"])){

    $_SESSION["goruntuleme_klasor"] = $_GET["klasor"];

    $goruntulenecek_klasor = $_GET["klasor"];

}else{

    if(isset($_SESSION["goruntuleme_klasor"])){

        $goruntulenecek_klasor = $_SESSION["goruntuleme_klasor"];
...
if(isset($_GET["hesap_no"])){

    $_SESSION["hesap_no"] = $_GET["hesap_no"];

    $account_no = $_GET["hesap_no"];

}else{

    if(isset( $_SESSION["hesap_no"])){

        $account_no = $_SESSION["hesap_no"];
    }
}

$configs  = include("../manager/config.php");
...
try {
    ...
    if (isset($_SESSION["extra_config"]["db"]["database"])) {
        ...
        $dbconfig["database"] = $_SESSION["extra_config"]["db"]["database"];
    }
    ...
    echo "Veritabanı Bağantı Hatası!";
...
if($conn != NULL){
    ...
    $sql = "SELECT media_key FROM hesap_detaylari WHERE account_id = :account_no  ";

    $sth =$conn->prepare($sql);

    $sth->bindParam(':account_no', $account_no);
    ...
    if($result){
        ...
        if(trim($result["media_key"]) == ""){
            echo "hatalı media kodu!";
            die();
        }else{

            $private_key_avokado = md5($result["media_key"]);
        }
    ...
    $sth =$conn->prepare( "SELECT paket_tanimlama_1 FROM hesap_paketleri WHERE hesap_no = ? and paket_key = ? ");

    $sth->execute([$account_no,"depolama"]);

    $depolama_result = $sth->fetch();

    if($depolama_result){


        $toplam_tanimli_depolama_alani = $depolama_result["paket_tanimlama_1"];
...
$current_path_avok = '../../../storage/'.$account_no.'/media/'.$goruntulenecek_klasor.'/';

$thumbs_base_path_avok =  '../../../storage/'.$account_no.'/thumbs/'.$goruntulenecek_klasor.'/';

$public_current_path_avok = '../../media/'.$account_no.'/s/';

$public_thumbs_base_path_avok =  '../../media/'.$account_no.'/t/';

$avok_upload_dir =  '/storage/';

$thumbs_upload_dir_avok = '/storage-thumb/';


if($goruntulenecek_klasor == "sistem") {

    $user_remove_file = false;
    $user_rename_file = false;
    $user_cretate_file = false;
    $user_upload_file = false;
    $user_move_file = false;
    $user_rename_folder = false;
    $user_edit_file = false;
    $user_upload_file = false;

    $klasor_limit = 0;

}else if($goruntulenecek_klasor == "private"){


    $public_folder_size =  folderSize($public_current_path_avok);

    $kalan_depolama = $toplam_tanimli_depolama_alani - $public_folder_size ;

    $kalan_depolama = (int) byteFormat($kalan_depolama,"MB");
    ...
    if($kalan_depolama <= 0){
        ...
        $klasor_limit =  $kalan_depolama;
    }



}else if($goruntulenecek_klasor == "public"){


    $base_url = $configs["mediaUrl"];

    $current_path_avok =  $public_current_path_avok;

    $thumbs_base_path_avok = $public_thumbs_base_path_avok;

    $avok_upload_dir =  '/'.$account_no.'/s/';

    $thumbs_upload_dir_avok = '/'.$account_no.'/t/';
}
...
$config = array(
    ...
    */
    ...
    |
    */
    'current_path' => $current_path_avok,
    ...
    |
    */
    'thumbs_base_path' => $thumbs_base_path_avok,
    ...
    |
    */
    'thumbs_upload_dir' => $thumbs_upload_dir_avok,
    ...
    */
    ...
    'access_keys' => array($private_key_avokado),
    ...
    */
    ...
    //******************
    'delete_files'                            => $user_remove_file,
    'create_folders'                          => $user_upload_file,
    'delete_folders'                          => $user_remove_file,
    'upload_files'                            => $user_upload_file,
    'rename_files'                            => $user_rename_file,
    'rename_folders'                          => $user_rename_folder,
    ...
    'edit_text_files'                         => $user_edit_file, // eg.: txt, log etc.
    'create_text_files'                       => $user_cretate_file, // only create files with exts. defined in $config['editable_text_file_exts']
    ...
    */
    ...
    'relative_image_creation_name_to_append'  => array( '_1thumb', '_2thumb' ), //name to append on filename
    'relative_image_creation_width'           => array( 500, 600 ), //width of image
